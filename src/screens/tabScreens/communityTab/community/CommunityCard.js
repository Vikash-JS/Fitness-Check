...
const CommunityCard = (props) => {
    return (
        <View style={styles.mainComtainer}>
            <View style={{justifyContent:'center',alignItems:'center',backgroundColor:Colors.Goal_BorderGrey}}>
                <Image style={{height:158, width:props.item.communityId.coverImage? '100%' : '50%', borderRadius: 10 }} resizeMode='cover' source={props.item.communityId.coverImage?{ uri : props.item.communityId.coverImage}:imagesFile.ic_NoImage} />
            </View>
            <View style={{ position: 'absolute', bottom: 15, marginLeft: 14 ,width:'100%'}}>
                <View>
                    <Text style={styles.bold_18_white}>{props.item.communityId.title}</Text>
                </View>
